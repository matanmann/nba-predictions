...
export default function DashboardClient({ year }: { year: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12">
      ...
      {activeTab === 'Leaderboard' && <LeaderboardView predictions={d.predictions ?? []} playoffLeaders={d.playoffLeaders ?? []} />}
      {activeTab === 'Bracket' && <BracketView series={d.series ?? []} />}
...
function LeaderboardView({ predictions, playoffLeaders }: { predictions: Prediction[]; playoffLeaders: { category: string; playerName: string }[] }) {
          ...
          return (
            <div key={p.userId} className={`flex items-center justify-between px-4 py-3 rounded-xl border ${rank <= 3 ? 'bg-white border-gray-200' : 'bg-gray-50 border-gray-100'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-800">{p.userId}</p>
...
function BracketView({ series }: { series: Series[] }) {
